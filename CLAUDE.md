...
**Kernel Construction (`_init_matrices`)**: Toeplitz/log-grid construction. With log-spaced CR and photon grids sharing the same bins/decade, every integration corner `y = b_ph[j] · E_mo[i_mo] / m_p` and `x = b_cr[i_da] / E_mo[i_mo]` lies on a 1D log-grid indexed by `(i_mo + j)` and `(i_da - i_mo)` respectively. Each response spline is sampled **once** on a length-`(dcr+dph)` y-grid (1D) or a `(dcr+dph) × 2 dcr` (y, x) outer-product grid (2D); per-channel tiles are then assembled by integer-index lookup. The prefactor `int_fac · diff_fac` simplifies algebraically to `m_p / E_mo[i_mo]` (bc) or that times `Δec[i_mo] / Δec[i_da]` (diff) — never materialised as full (dcr, dph) tensors. Measured on production grid (8 bins/decade), scipy backend, M-series Mac, isolating `_init_matrices` from the shared `resp` setup: 3.8 s at max_mass=14, 4.3 s at max_mass=28 (~3–5.5× faster than the dense-tensor implementation it replaced).

`_assert_log_grids_compatible` runs at the start of `_init_matrices` and **raises a `RuntimeError`** if `cosmic_ray_grid` and `photon_grid` (or the CR center/edge log-steps) do not all share the same bins/decade. There is no silent fallback — mismatched grids would silently produce wrong rates, so the user must align the grids.

`_batch_matrix` is stored densely; persistent storage is unchanged from the previous implementation. Per-channel Toeplitz storage with lazy expansion in `_update_rates` is a possible future memory optimization, not implemented.
